function [CR] = RightMoveEvn(ix, CR, A, B)
...
CR = ncon({CR, conj(B{ix,7}), A{ix,7}, conj(B{ix,8}), A{ix,8}}, {[-1,-2,3,5,6,7], [8,-3,-5,2,6], [8,-4,-6,4,7], [1,2,3], [1,4,5]}, [3,5,1,2,6,4,7,8]);

CR = reshape(CR, [D2, Db, Da, Db, Da, D2]);
CR = ncon({CR, conj(B{ix,5}), A{ix,5}, conj(B{ix,6}), A{ix,6}}, ...
     {[-1,1,2,4,5,-6], [3,-2,1], [3,-3,2], [6,-4,4], [6,-5,5]}, [1,2,3,4,5,6]);    
CR = ncon({CR, conj(B{ix,4}), A{ix,4}}, {[-1,1,3,2,4,-6], [5,-2,-4,1,2], [5,-3,-5,3,4]}, [1,2,3,4,5]);
...
CR = ncon({CR, conj(B{ix,1}), A{ix,1}}, {[1,3,2,4,-5,-6], [5,-1,-3,1,2], [5,-2,-4,3,4]}, [1,2,3,4,5]);
