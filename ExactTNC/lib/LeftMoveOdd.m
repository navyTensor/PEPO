function [CL] = LeftMoveOdd(ix, CL, A, B)
...
CL = ncon({CL, conj(B{ix,8}), A{ix,8}}, {[-1,-2,1,3,2,4], [5,1,2,-3,-5], [5,3,4,-4,-6]}, [1,2,3,4,5]);
CL = ncon({CL, conj(B{ix,7}), A{ix,7}, conj(B{ix,9}), A{ix,9}}, ...
     {[-1,-2,1,2,4,5], [3,1,-3], [3,2,-4], [6,4,-5], [6,5,-6]}, [1,2,3,4,5,6]);

CL = reshape(CL, [D2, Db, Da, Db, Da, D2]);
CL = ncon({CL, conj(B{ix,5}), A{ix,5}}, {[-1,1,3,2,4,-6], [5,1,2,-2,-4], [5,3,4,-3,-5]}, [1,2,3,4,5]);
CL = ncon({CL, conj(B{ix,4}), A{ix,4}, conj(B{ix,6}), A{ix,6}}, ...
     {[-1,1,2,4,5,-6], [3,1,-2], [3,2,-3], [6,4,-4], [6,5,-5]}, [1,2,3,4,5,6]);

CL = reshape(CL, [Db, Da, Db, Da, D2, D2]);
CL = reshape(ncon({CL, conj(B{ix,2}), A{ix,2}, conj(B{ix,3}), A{ix,3}}, ...
