function [CL] = LeftMoveEvn(ix, CL, A, B)
...
CL = ncon({CL, conj(B{ix,1}), A{ix,1}}, {[1,3,2,4,-5,-6], [5,1,2,-1,-3], [5,3,4,-2,-4]}, [1,2,3,4,5]);
CL = ncon({CL, conj(B{ix,2}), A{ix,2}, conj(B{ix,3}), A{ix,3}}, ...
     {[1,2,4,5,-5,-6], [3,1,-1], [3,2,-2], [6,4,-3], [6,5,-4]}, [1,2,3,4,5,6]);

CL = reshape(CL, [D2, Db, Da, Db, Da, D2]);
CL = ncon({CL, conj(B{ix,4}), A{ix,4}}, {[-1,1,3,2,4,-6], [5,1,2,-2,-4], [5,3,4,-3,-5]}, [1,2,3,4,5]);
CL = ncon({CL, conj(B{ix,5}), A{ix,5}, conj(B{ix,6}), A{ix,6}}, ...
     {[-1,1,2,4,5,-6], [3,1,-2], [3,2,-3], [6,4,-4], [6,5,-5]}, [4,5,6,1,2,3]);

CL = reshape(CL, [D2, D2, Db, Da, Db, Da]);
CL = ncon({CL, conj(B{ix,7}), A{ix,7}, conj(B{ix,8}), A{ix,8}}, ...
