function [A, Td, EvoX, EvoZ] = InitFig3bPEPS(theta, depth)
...
Xd1 = reshape(ncon({conj(T0{1}),T0{1}, Sx}, {[-1,1,-3], [-2,2,-4], [1,2]}, [1,2]),[4,4]);
Xd2 = reshape(ncon({conj(T0{2}),T0{2}, Sx}, {[-1,1,-3,-5], [-2,2,-4,-6], [1,2]}, [1,2]),[4,4,4]);               % Ad2[m,a,b]
Yd2 = reshape(ncon({conj(T0{2}),T0{2}, Sy}, {[-1,1,-3,-5], [-2,2,-4,-6], [1,2]}, [1,2]),[4,4,4]);               % Ad2[m,a,b]
Zd2 = reshape(ncon({conj(T0{2}),T0{2}, Sz}, {[-1,1,-3,-5], [-2,2,-4,-6], [1,2]}, [1,2]),[4,4,4]);               % Ad2[m,a,b]
Yd3 = reshape(ncon({conj(T0{3}),T0{3}, Sy}, {[-1,1,-3,-5,-7], [-2,2,-4,-6,-8], [1,2]}, [1,2]),[4,4,4,4]);
...
for i = 2 : depth
    ...
    Xd1 = reshape(ncon({Td{1}, Xd1}, {[-1,1,-2], [1,-3]}),[4,Da]);                     % A1[m,a]
    ...
    Yd3 = reshape(ncon({Td{3}, Yd3}, {[-1,1,-2,-4,-6], [1,-3,-5,-7]}),[4,Da,Da,Da]);   % A3[m,a,b,c]
