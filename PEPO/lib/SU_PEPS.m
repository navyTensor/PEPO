function [A, Lam, TErr, Coef] = SU_PEPS(D, A, Lam, Rx, Rzz)
...
if( isempty(Evo) )
    if( ib == 1 )
        ...
        M = ncon({A3, B2, diag(LamA{1}), diag(LamA{2}), diag(LamAB), diag(LamB)},...
            {[-1,1,2,3], [-4,5,4], [-2,2], [3,-3], [4,1], [-5,5]}, [2,3,5,4,1]);
    elseif( ib == 2 )
        M = ncon({A3, B2, diag(LamA{1}), diag(LamA{2}), diag(LamAB), diag(LamB)},...
            {[-1,1,2,3], [-4,5,4], [-2,1], [3,-3], [4,2], [-5,5]}, [1,3,5,4,2]);
    elseif( ib == 3 )
        M = ncon({A3, B2, diag(LamA{1}), diag(LamA{2}), diag(LamAB), diag(LamB)},...
            {[-1,1,2,3], [-4,4,5], [-2,1], [-3,2], [3,4], [5,-5]}, [1,2,5,4,3]);
    end
else
    if( ib == 1 )
        if( dir == "dn" )
        ...
        M = ncon({A3, B2, diag(LamA{1}), diag(LamA{2}), diag(LamAB), diag(LamB), Evo},...
            {[1,7,2,3], [4,5,6], [-2,2], [3,-3], [6,7], [-5,5], [-1,-4,1,4]}, [2,3,7,5,6,1,4]);
    elseif( ib == 2 )
        M = ncon({A3, B2, diag(LamA{1}), diag(LamA{2}), diag(LamAB), diag(LamB), Evo},...
            {[1,2,7,3], [4,5,6], [-2,2], [3,-3], [6,7], [-5,5], [-1,-4,1,4]}, [2,3,7,5,6,1,4]);
    elseif( ib == 3 )
        M = ncon({A3, B2, diag(LamA{1}), diag(LamA{2}), diag(LamAB), diag(LamB), Evo},...
            {[1,2,3,7], [4,6,5], [-2,2], [-3,3], [7,6], [5,-5], [-1,-4,1,4]}, [2,3,7,5,6,1,4]);
    end
end
...
M = reshape(M, [Dm(1)*Dm(2)*Dm(3), Dm(4)*Dm(5)]);
[U, S, V] = svd(M, 'econ');
S = diag(S);
Coef = S(1);
S = S/Coef;
...
V = reshape(V(:, 1:Dnew), [Dm(4), Dm(5), Dnew]);
LamAB = S(1:Dnew);
...
if( ib == 1 )
    A3 = ncon({U, diag(invLamA0), diag(invLamA1)}, {[-1,1,2,-2], [-3,1], [2,-4]}, [1,2]);
    B2 = ncon({conj(V), diag(invLamB)}, {[-1,1,-3], [-2,1]});
elseif( ib == 2 )
    A3 = ncon({U, diag(invLamA0), diag(invLamA1)}, {[-1,1,2,-3], [-2,1], [2,-4]}, [1,2]);
    B2 = ncon({conj(V), diag(invLamB)}, {[-1,1,-3], [-2,1]});
elseif( ib == 3 )
