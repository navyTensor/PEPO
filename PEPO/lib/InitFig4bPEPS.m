function [A, Td, EvoX, EvoZ] = InitFig4bPEPS(theta, depth)
% A[m,u,l,d,r], Uz = (Rz)^{+}, Ux = (Rx)^{+}
% <0|(UxUz)^19 UxUz(Zk)RzRx (RzRx)^19|0> = <0|(UxUz)^19 Ux(Zk)Rx (RzRx)^19|0> 
%=<0|UxUz(UxUz)^17 UxUzUx(Zk)RxRzRx(RzRx)^17 RzRx|0> =<0|Ux(UzUx)^19 (Zk)(RxRz)^19 Rx|0> 
...
Z62 = reshape(ncon({conj(T{3}),T{3}, Sz}, {[-1,1,-3,-5,-7], [-2,2,-4,-6,-8], [1,2]}, [1,2]),[4,4,4,4]);
...
for i = 2 : depth
    ...
    Z62 = reshape(ncon({Td{3}, Z62}, {[-1,1,-2,-4,-6], [1,-3,-5,-7]}),[4,Da,Da,Da]);   % A3[m,a,b,c]
end
